...
<html lang="en">
    ...
    <body>
        ...
        <div class="navbar navbar-fixed-top navbar-inverse">
            <div class="navbar-inner">
                <div class="container-fluid">
                    <a class="brand" href="#">Auth8 Docs</a>
                </div>
            </div>
        </div>
        ...
        <div class="container-fluid">
            <div class="row-fluid">
                ...
                <div class="span3">
                    ...
                    <div id="authdiv" style="display:none;"></div>
        ...
        <script id="auth" type="text/x-handlebars-template">
            ...
            <span style="text-align:left;" class="add-on span4">Public Key</span><input id="inputPublicKey" type="text" class="span8" name="Public Key" placeholder="Public Key">
            </div>
            <div class="input-prepend">
            <span style="text-align:left;" class="add-on span4">Private Key</span><input id="inputPrivateKey" type="password" class="span8" name="Private Key" placeholder="Private Key">
            </div>
            <br>
        </script>
...
$(document).ready(
        function fetchdoc() {
            $.ajax({
                url: window.location.protocol + '/api/jsondoc',
                type: 'GET',
                dataType: 'json',
                ...
                success: function (data) {
                    ...
                    var auth = Handlebars.compile($("#auth").html());
                    var authHTML = auth(data);
                    $("#authdiv").html(authHTML);
                    $("#authdiv").show();
                            ...
                            $('#content a[rel="method"]').each(function () {
                                $(this).click(function () {
                                    ...
                                    $("#testButton").click(function () {
                                        ...
                                        var tempReplacedPath = replacedPath;
                                        ...
                                        $("#queryparameters input").each(function () {
                                            if ($(this).val().length > 0) {
                                                // Add the '?' if its the first
                                                if (replacedPath.indexOf("?") === -1) {
                                                    tempReplacedPath = replacedPath + "?" + this.name + "=" + encodeURIComponent($(this).val());
                                                } else {
                                                    tempReplacedPath = replacedPath + "&" + this.name + "=" + encodeURIComponent($(this).val());
                                                }
                                            }
                                        ...
                                        if (replacedPath.indexOf("?") === -1) {
                                            tempReplacedPath = replacedPath + "?timestamp=" + new Date().getTime();
                                        } else {
                                            tempReplacedPath = replacedPath + "&timestamp=" + new Date().getTime();
                                        }
                                        replacedPath = tempReplacedPath;

                                        // Add in the public key
                                        tempReplacedPath = replacedPath + "&public_key=" + $("#inputPublicKey").val();
                                        replacedPath = tempReplacedPath;

                                        // Create the signature
                                        var shaObj = new jsSHA(model.basePath + replacedPath, "TEXT");
                                        var hmac = shaObj.getHMAC($("#inputPrivateKey").val(), "TEXT", "SHA-1", "B64");
                                        tempReplacedPath = replacedPath + "&signature=" + encodeURIComponent(hmac);
                                        replacedPath = tempReplacedPath;
